...
- **Javadoc**: Static Java API documentation generated for each microservice.
...
To view Javadoc locally, open these files in your browser:

- **ms-accounts:** `ms-accounts/target/site/apidocs/index.html`
- **ms-customers:** `ms-customers/target/site/apidocs/index.html`

From CI, you can also download the artifacts generated by `generate-javadocs.yml` and open `index.html`:

- `javadocs-ms-accounts`
- `javadocs-ms-customers`

## Postman Collection 📬

A Postman collection with example requests for all implemented endpoints will be provided in the root folder of the repository. This collection will include sample payloads, expected responses, and environment variables for easy testing.
...
- **Generate Javadocs** (`generate-javadocs.yml`): Generates Javadoc for `ms-accounts` and `ms-customers` and publishes them as workflow artifacts.
